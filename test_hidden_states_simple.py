...
def test_data_structures_flow():
    ...
    try:
        ...
        print(f"❌ Data structures flow test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def test_zmq_pipeline_structures():
    ...
    try:
        ...
        print(f"❌ ZMQ pipeline structures test failed: {e}")
        import traceback
        traceback.print_exc()
        return False
...
def main():
    ...
    all_passed &= test_hidden_states_model_runner()
    all_passed &= test_data_structures_flow()
    all_passed &= test_zmq_pipeline_structures()
    
    print("=" * 50)
    if all_passed:
        print("🎉 All tests PASSED! Hidden states implementation is working.")
        print()
        print("📋 Implementation Status:")
        print("✅ Data structures extended (EngineCoreRequest, ModelRunnerOutput, etc.)")
        print("✅ Model forward pass integration implemented")
        print("✅ ZMQ pipeline data structures working")
        print("🔄 ZMQ client logic in OutputProcessor pending")
        print("🔄 End-to-end ZMQ pipeline pending")
    else:
